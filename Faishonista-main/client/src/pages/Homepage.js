...
const HomePage = () => {
  ...
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(false);
  const [showFilters, setShowFilters] = useState(false); // Sidebar state
  ...
  return (
    <Layout title={"All Products - Best Offers"}>
      ...
      <div className={`filter-sidebar ${showFilters ? "show" : ""}`}>
        {/* Close Button */}
        <button className="close-sidebar-btn" onClick={() => setShowFilters(false)}>
          <AiOutlineClose />
        </button>
      ...
      <div className="container-fluid row mt-3 home-page">
        <div className="col-md-9">
          ...
          <div className="d-flex flex-wrap">
            {products?.map((p) => (
              <div style={{left:"16.5rem" }} className="card m-2 " key={p._id}>
                <img
                  src={`/api/v1/product/product-photo/${p._id}`}
                  className="card-img-top"
                  alt={p.name}
